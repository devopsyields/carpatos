/* pkg.c — incarcare / salvare fisier .cpm (antet + manifest + payload) */
#define _GNU_SOURCE
#include "cpm.h"
...
int cpm_incarca(const char *cale, Manifest *m,
    ...
    if (!buf) { cpm_err("nu pot citi %s", cale); return -1; }
    if (total < 16) {
        free(buf); cpm_err("%s: fisier prea mic pentru .cpm", cale); return -1;
    }
    ...
    if (magic != CPM_MAGIC) {
        free(buf);
        cpm_err("%s: magic invalid (0x%08x)", cale, magic);
        return -1;
    }
    if (ver != CPM_FORMAT_VER) {
        free(buf);
        cpm_err("%s: versiune format %u necunoscuta", cale, ver);
        return -1;
    }
    if ((size_t)16 + mlen + plen > total) {
        free(buf);
        cpm_err("%s: dimensiuni incorecte in antet", cale);
        return -1;
    }

    if (manifest_parseaza(buf + 16, mlen, m) < 0) {
        free(buf);
        cpm_err("%s: manifest invalid", cale);
        return -1;
    }
...
int cpm_salveaza(const char *cale, const Manifest *m,
    ...
    if (mlen < 0) { cpm_err("manifest prea mare pentru serializare"); return -1; }

    FILE *f = fopen(cale, "wb");
    if (!f) { cpm_err("nu pot scrie %s", cale); return -1; }

    unsigned char header[16];
    pune_u32(header,      CPM_MAGIC);
    pune_u32(header + 4,  CPM_FORMAT_VER);
